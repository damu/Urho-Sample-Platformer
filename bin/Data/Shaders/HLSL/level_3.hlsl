...
{
    float f;
    f=snoise(p.xyz*0.1)*2+
      snoise(p.xyz*0.5)*0.5+
      snoise(p.xyz*5)*0.05;
    f=0.5+f*0.2;
    f=clamp(f,0,1);
    //f-=0.5;
    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    float f2=noise_height(iWorldPos.xyz+float3(-0.05,0,0));
    float f3=noise_height(iWorldPos.xyz+float3(0,-0.05,0));
    float3 va=float3(0,(f2-f)*32,1);
    float3 vb=float3(1,(f3-f)*32,0);
    normal+=normalize(cross(va,vb));
    normal=normalize(normal);

    f=noise(iWorldPos.xyz);
    float4 spec_color = float4(cMatSpecColor.rgb*f,cMatSpecColor.a);
    float4 diffColor=float4(f*0.2,f*0.18,f*0.16,1);
