...
void VS(float4 iPos : POSITION,
    ...
    #endif
    out float4 vScreenPos : TEXCOORD8,
    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    float dist_fix=100-screenPos.w;    // make the bumps weaker depending on distance to avoid flickering
    dist_fix=clamp(dist_fix/100.0,0.0,1.0);
    float f=noise_height_normal(iWorldPos.xyz);
    float f2=noise_height_normal(iWorldPos.xyz+float3(-0.05,0,0));
    float f3=noise_height_normal(iWorldPos.xyz+float3(0,-0.05,0));
    float3 va=float3(0,(f2-f),1);
    float3 vb=float3(1,(f3-f),0);
    normal+=normalize(cross(va,vb))*dist_fix;
    normal=normalize(normal);

    f=noise_height(iWorldPos.xyz);
    float4 spec_color=cMatSpecColor*f;
    spec_color.a=cMatSpecColor.a;
    float4 diffColor=cMatDiffColor*f;
